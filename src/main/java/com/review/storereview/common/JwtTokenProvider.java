    ...
    public String createTokenFromAuthentication(Authentication authentication) throws Exception
    {
        ...
        try {
            jwt = Jwts.builder()
                    ...
                    //.claim("suid", CryptUtils.Base64Encoding(cryptUtils.getAES().encrypt(cryptUtils.getSecretKey(), ((JWTUserDetails) authentication.getPrincipal()).getSuid())))
                    //.claim("said", CryptUtils.Base64Encoding(cryptUtils.getAES().encrypt(cryptUtils.getSecretKey(), ((JWTUserDetails) authentication.getPrincipal()).getSaid())))
